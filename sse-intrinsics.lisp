...
(def-load-intrinsic cpu-prefetch-t0 nil prefetch "_mm_prefetch" :tags (:t0) :size :byte :postfix-fmt ",_MM_HINT_T0" :side-effect? t)
(def-load-intrinsic cpu-prefetch-t1 nil prefetch "_mm_prefetch" :tags (:t1) :size :byte :postfix-fmt ",_MM_HINT_T1" :side-effect? t)
(def-load-intrinsic cpu-prefetch-t2 nil prefetch "_mm_prefetch" :tags (:t2) :size :byte :postfix-fmt ",_MM_HINT_T2" :side-effect? t)
(def-load-intrinsic cpu-prefetch-nta nil prefetch "_mm_prefetch" :tags (:nta) :size :byte :postfix-fmt ",_MM_HINT_NTA" :side-effect? t)

(def-load-intrinsic cpu-clflush nil clflush "_mm_clflush" :size :byte :side-effect? t)
...
(progn
  ...
  (define-vop (%maskmoveu-pi)
    ...
    (:args (value :scs #.+any-sse-reg+)
           (mask :scs #.+any-sse-reg+)
           (sap :scs (sap-reg) :target rdi)
           (offset :scs (signed-reg)))
    (:arg-types simd-pack simd-pack system-area-pointer signed-num)
  ...
  (define-vop (%maskmoveu-pi-c)
    ...
    (:args (value :scs #.+any-sse-reg+)
           (mask :scs #.+any-sse-reg+)
           (sap :scs (sap-reg) :target rdi))
    (:arg-types simd-pack simd-pack system-area-pointer (:constant (signed-byte 32)))
    (:info offset)
    (:temporary (:sc sap-reg :offset rdi-offset :from :eval) rdi)
    (:policy :fast-safe)
    (:note "inline MASKMOVEU operation")
...
(macrolet ((defimm (name insn bits &key arithmetic)
             `(define-vop (,(symbolicate "%" name "-IMM") sse-int-base-op)
                ...
                (:arg-types simd-pack (:constant fixnum))
                (:result-types sb-kernel:simd-pack-int)
                (:info immv)
                (:generator 1
                  ,@(let ((core `(progn
